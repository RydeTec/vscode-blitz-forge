import { builtinFunctionsLower } from "../context/context";
...
export function isBlitzKeyword(name: string): boolean {
    return ',if,then,else,elseif,endif,select,case,default,end,and,or,xor,not,repeat,until,forever,while,wend,for,to,step,next,exit,goto,gosub,return,function,method,const,global,local,dim,type,field,new,each,first,last,insert,delete,true,false,null,data,read,restore,include,pi,mod,shl,shr,sar,'.includes(',' + name.toLowerCase() + ',');
}
...
export function isBuiltinBlitzType(type: string): boolean {
    return ',bbthread,bbbank,bblist,bbstream,bbchannel,bbsoundbbtexture,bbbrush,bbsurface,bbentity,bbbuffer,bbimage,bbfont,bbdir,bbpointer,bbfunction,'.includes(',' + type.toLowerCase() + ',');
}
